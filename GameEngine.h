...
public:
	void Start()
	{
		// Start the thread
		m_bAtomActive = true;
		std::thread t = std::thread(&CustomGameEngine::GameThread, this);
...
protected: // Audio Engine =====================================================================
		...
		unsigned int nBlocks = 8, unsigned int nBlockSamples = 512)
	{
		...
		m_AudioThread = std::thread(&CustomGameEngine::AudioThread, this);
	...
	{
		((CustomGameEngine*)dwInstance)->waveOutProc(hWaveOut, uMsg, dwParam1, dwParam2);
	}
...
std::atomic<bool> CustomGameEngine::m_bAtomActive(false);
std::condition_variable CustomGameEngine::m_cvGameFinished;
std::mutex CustomGameEngine::m_muxGame;